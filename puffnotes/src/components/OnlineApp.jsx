...
export default function OnlineApp({ user, accessToken, folderId, onSignOut }) {
  ...
  // --- Functions for Online Mode ---
  const refreshFileList = async () => {
    try {
      const files = await listNotes(accessToken, folderId);
      setFileList(files || []);
      return files;
    } catch (err) {
  ...
  const handleOpenFile = async (file) => {
    if (!file || isLoadingNote) return;
    setIsLoadingNote(true);
    setShowFileModal(false);
    try {
      const content = await getNoteContent(accessToken, file.id);
  ...
  const handleAutoSave = async (isNewNote = false) => {
    ...
    try {
      ...
      if (!activeNoteId) {
        setActiveNoteId(savedFile.id);
      }
      
      // THE FIX: Always refresh the file list after a successful save.
  ...
  // --- Beautify, PDF Export, and UI Functions (mostly unchanged) ---
  ...
  }, [note, noteName, saveStatus, isInitialLoad]);
  ...
  const handleNoteNameChange = (e) => {
   ...
   useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      else if (ctrlOrCmd && e.key.toLowerCase() === 's') { e.preventDefault(); if(saveStatus === 'unsaved') handleAutoSave(); }
      else if (ctrlOrCmd && e.shiftKey && e.key.toLowerCase() === 'f') { e.preventDefault(); toggleFocusMode(); }
      else if (ctrlOrCmd && e.key.toLowerCase() === 'o') { e.preventDefault(); setShowFileModal(p => !p); }
  ...
  }, [note, isBeautifying, showBeautifyControls, saveStatus, handleBeautify, handleExportPdf, handleNewNote, handleAutoSave, toggleFocusMode, setShowShortcutsModal]);
