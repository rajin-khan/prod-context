...
export default function App() {
  ...
  // --- Functions (No Changes to existing functions) ---
  ...
  // --- useEffect Hooks (No Changes to logic inside hooks) ---
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (!isAppVisible) return; // <-- Disable shortcuts on landing page
  ...
  }, [ isAppVisible, note, isBeautifying, showBeautifyControls, handleBeautify, handleExportPdf, handleNewNote, handleSave, toggleFocusMode, handleFolderButton, setShowShortcutsModal ]);

  // --- Render Logic ---
  return (
    <>
      <AnimatePresence>
        {!isAppVisible && <LandingPage onStart={() => setIsAppVisible(true)} />}
      </AnimatePresence>

      <div className={`min-h-screen bg-[#fdf6ec] relative overflow-hidden transition-opacity duration-500 ${isAppVisible ? 'opacity-100' : 'opacity-0'}`}>
